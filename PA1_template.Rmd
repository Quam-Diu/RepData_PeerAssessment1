...
```{R plot.histogram, fig.align='center'}
...
```{R plot.timeSeries, fig.align='center'}
...
```{R plot.newHistogram, fig.align='center'}
        ## 1) Calculates the total number of steps per day
        ## Days with no data recorded (only NAs on the steps column) were exclude
...
```{R plot.weekday, fig.align='center'}
...
From this two plots, we can observe that the shape of the time series is different from weekdays to weekends:  
 ...
 - The number of time intervals with a mean above 100 steps on weekends is `r weekend.100`, while on weekdays is `r weekday.100`.  
 - Finally, the mean steps on weekdays is `r weekday.mean`, meanwhile on weekends is `r weekend.mean`.
